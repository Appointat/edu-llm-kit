...
class CSSGenerator(ABC):
    @abstractmethod
    def generate(self) -> str:
        pass


class OrangeThemedCSSGenerator(CSSGenerator):
    def __init__(self):
        pass

    def _create_base_rules(self) -> List[CSSRule]:
        return [
            CSSRule(
                "body",
                {
            ...
            CSSRule(
                ...
                {"fill": "#ffe4cc", "stroke": "#ff8c00", "stroke-width": "2px"},
            ),
            CSSRule(
                ".node.decision rect, .node.decision circle, .node.decision ellipse, .node.decision polygon",
                {"fill": "#ffd700", "stroke": "#ff8c00", "stroke-width": "2px"},
            ),
            CSSRule(".node text", {"fill": "#663300", "font-weight": "bold"}),
            CSSRule(
                ".edgePath path", {"stroke": "#ff8c00", "stroke-width": "2px"}
            ),
            CSSRule(".edgeLabel rect", {"fill": "#fff9f5", "opacity": "0.7"}),
            CSSRule(".edgeLabel text", {"fill": "#663300"}),
            CSSRule(
                ".cluster rect",
                {"fill": "#fff0e6", "stroke": "#ff8c00", "stroke-width": "1px"},
            ),
            CSSRule(".cluster text", {"fill": "#663300"}),
        ]

    def generate(self) -> str:
        css_rules = self._create_base_rules()

        css_text = ""
        for rule in css_rules:
            css_text += f"{rule.selector} {{\n"
...
class MermaidOrangeThemedHTMLGenerator:
    def __init__(self, mermaid_code: str):
        self.mermaid_code = mermaid_code
        self.css_generator = OrangeThemedCSSGenerator()
...
graph TD
    ...
    generator = MermaidOrangeThemedHTMLGenerator(mermaid_code)
    html_content = generator.generate_html()

    with open("orange_themed_ai_tech_mermaid_diagram.html", "w") as f:
        f.write(html_content)

    print(
        "HTML file 'orange_themed_ai_tech_mermaid_diagram.html' has been generated successfully."
    )
